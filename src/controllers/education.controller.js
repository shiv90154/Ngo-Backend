// backend/src/controllers/education.controller.js
...
exports.createCourse = catchAsync(async (req, res, next) => {
  const { title, description, price, category, language, isPublished } = req.body;
...
exports.addChapter = catchAsync(async (req, res, next) => {
  ...
  if (!course) return next(new AppError('Unauthorized or course not found', 403));
...
exports.addLesson = catchAsync(async (req, res, next) => {
  ...
  const lesson = await Lesson.create({ chapter: chapterId, title, type, content, order, isPreview });
...
exports.createTest = catchAsync(async (req, res, next) => {
  ...
  const course = await Course.findOne({ _id: courseId, instructor: req.user.id });
  if (!course) return next(new AppError('Unauthorized', 403));
  const test = await Test.create({ course: courseId, title, description, duration, totalMarks, passingMarks });
...
exports.addQuestion = catchAsync(async (req, res, next) => {
  ...
  const question = await Question.create({ test: testId, questionText, options, correctAnswer, marks, explanation });
  res.status(201).json({ success: true, question });
});

// ====================== STUDENT BROWSE & ENROLLMENT ======================
exports.getPublishedCourses = catchAsync(async (req, res, next) => {
  ...
  res.json({ success: true, courses, currentPage: parseInt(page), totalPages: Math.ceil(total / limit), total });
});
...
exports.enrollCourse = catchAsync(async (req, res, next) => {
  ...
  if (enrollment) return res.json({ success: true, enrollment, message: 'Already enrolled' });
...
exports.getMyEnrollments = catchAsync(async (req, res, next) => {
  ...
  const enrollments = await Enrollment.find({ student: req.user.id })
    .populate({ path: 'course', select: 'title description thumbnail price instructor totalEnrolled', populate: { path: 'instructor', select: 'fullName' } })
    .sort('-enrolledAt')
    .skip((page - 1) * limit)
    .limit(parseInt(limit));
  const total = await Enrollment.countDocuments({ student: req.user.id });
  res.json({ success: true, enrollments, currentPage: parseInt(page), totalPages: Math.ceil(total / limit) });
});

exports.getMyCertificates = catchAsync(async (req, res, next) => {
  const certificates = await Certificate.find({ student: req.user.id }).populate('course', 'title').sort('-issuedAt');
...
exports.startTest = catchAsync(async (req, res, next) => {
  ...
  if (!enrollment) return next(new AppError('Enroll first', 403));

  const existingAttempt = await TestAttempt.findOne({ test: test._id, student: req.user.id, submittedAt: { $exists: false } });
  if (existingAttempt) return res.json({ success: true, attempt: existingAttempt, message: 'Previously started test' });

  const attempt = await TestAttempt.create({ test: test._id, student: req.user.id, startedAt: new Date() });
...
exports.submitTest = catchAsync(async (req, res, next) => {
  ...
  if (!attempt || attempt.student.toString() !== req.user.id) return next(new AppError('Invalid attempt', 403));
  if (attempt.submittedAt) return next(new AppError('Test already submitted', 400));
  ...
  if (passed) {
    const enrollment = await Enrollment.findOne({ student: req.user.id, course: attempt.test.course });
